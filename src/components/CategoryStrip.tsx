...
export default function CategoryStrip({ categories }: { categories: Category[] }) {
    ...
    const getIcon = (slug: string) => {
        const icons: Record<string, any> = {
            'disposable-vapes': <Cloud size={24} />,
            'pod-systems': <Zap size={24} />,
            'box-mods': <Battery size={24} />,
            'e-liquids': <Droplets size={24} />,
            'accessories': <Box size={24} />,
        };
        return icons[slug] || <Sparkles size={24} />;
    };
        ...
        <div style={{
            ...
            {categories.map((cat) => (
                <Link key={cat.id} href={`/${locale}/search?category=${cat.id}`} style={{ flexShrink: 0 }}>
                    <motion.div
                        whileHover={{ y: -4, borderColor: 'var(--border-glow)', boxShadow: 'var(--shadow-primary)' }}
                        whileTap={{ scale: 0.95 }}
                        style={{
                            display: 'flex',
